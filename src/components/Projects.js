    ...
    const fetchProjects = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/users/Llewellynhtml/repos"
        );
        if (!response.ok) {
      ...
      {projects.map((project) => (
        <div key={project.id} className={styles.project}>
          ...
          <p>
            ...
            {renderTechStack(project.language || "Not specified")}
          </p>

          <p>
            <strong>Challenges Faced and Solutions:</strong> Highlight a problem
